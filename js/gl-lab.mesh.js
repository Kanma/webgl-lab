...
/***************************** FUNCTION: Mesh instanciation *****************************/

//----------------------------------------------------------------------------------------
// Instanciate an object from a mesh
//----------------------------------------------------------------------------------------
gl_lab.mesh.instanciate = function(gl, name, mesh)
{
    ...
    for (var i = 0; i < submeshes_declaration.length; ++i)
    {
        ...
        {
            if (colors)
                submesh.setShaderProgram(gl, 'vertex_colors__no_lighting.vs', 'simple.fs');
            else
                submesh.setShaderProgram(gl, 'no_texture__no_lighting.vs', 'simple.fs');
        }
        else
        {
